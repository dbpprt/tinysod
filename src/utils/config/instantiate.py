...
from src.utils.config.locate import _locate
...
def instantiate_node(
    ...
    *args: Any,
    ...
    recursive: bool | Any = True,
    partial: bool | Any = False,
) -> Any:
    # Return None if config is None
    ...
    if OmegaConf.is_list(node):
        ...
        if _is_target(node):
            ...
            if convert == ConvertMode.ALL or (
                convert in (ConvertMode.PARTIAL, ConvertMode.OBJECT) and node._metadata.object_type in (None, dict)
            ):
                dict_items = {}
